  ...
  if (state.loadingVideoTasks) {
    return (
      <div className="flex items-center space-x-2 min-w-[120px] p-2 justify-center h-full">
        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-blue-600"></div>
        <span className="text-xs text-gray-600 dark:text-gray-400">Loading videos...</span>
      </div>
    );
  }
    ...
    <div className="flex items-center space-x-4 overflow-x-auto">
      {state.videoTasks.map((task) => (
        <button
          ...
          className={`flex flex-col items-center space-y-1 min-w-[120px] max-w-[140px] p-2 rounded-lg border-2 transition-all ${task.status === 'SUCCESS'
        ...
        >
          ...
          <div className="text-center w-full">
            <span className={`text-xs px-1 py-0.5 rounded text-white ${getStatusBadgeColor(task.status)}`}>
              {task.status}
            </span>
          </div>
        </button>
