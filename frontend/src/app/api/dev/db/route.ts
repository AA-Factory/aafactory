  ...
  try {
    ...
    if (resetType === 'all') {
      try {
        ...
        resetResults.errors.push(
          `Complete reset error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      }
    }
    ...
    if (resetType === 'selective') {
      ...
      if (collections.length > 0) {
        for (const collectionName of collections) {
          try {
            ...
            resetResults.errors.push(
              `Collection ${collectionName} reset error: ${error instanceof Error ? error.message : 'Unknown error'}`,
            );
          }
        }
      }
      ...
      if (folders.length > 0) {
        try {
          ...
          resetResults.errors.push(
            `Folders reset error: ${error instanceof Error ? error.message : 'Unknown error'}`,
          );
        }
      }
    }
    ...
    if (resetType === 'collections') {
      try {
        ...
        resetResults.errors.push(
          `Collections reset error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      }
    }
    ...
    if (resetType === 'files') {
      try {
        ...
        resetResults.errors.push(
          `Files reset error: ${error instanceof Error ? error.message : 'Unknown error'}`,
        );
      }
    }
